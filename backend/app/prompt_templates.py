...
通话结束信号（机器人电话专用，极其重要）：
当出现以下任意情形时，在当轮回复的最后加上 [HANGUP] 标记，系统将在播完该轮回复后自动挂断电话。禁止在其他情况下输出 [HANGUP]。
触发情形：客户明确表示不感兴趣或拒绝；客户要求挂断或表示忙碌；完成成交或预约确认并做完收尾致谢；对话已达成明确下一步行动且无需继续；客户情绪明显激动或出现辱骂。
示例：好的，感谢您的时间，祝您生活愉快。[HANGUP]
